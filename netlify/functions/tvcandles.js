  ...
  try {
    ...
    const r = await fetch(url, {
      headers: {
        ...
        'Cookie': 'sessionid=y2f7decnyu4zne1mi58s50vonuqnbrw4; sessionid_sign=v3:y2f7decnyu4zne1mi58s50vonuqnbrw4'
      }
    });
    if (!r.ok) throw new Error('TV ' + r.status);
    const d = await r.json();
    if (d.s !== 'ok') throw new Error('TV status: ' + d.s + (d.errmsg ? ' ' + d.errmsg : ''));
